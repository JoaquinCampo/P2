...
bool esAvl(TAbb abb)
{
  ...
  alturaIzq = altura(izquierdo(abb));
  alturaDer = altura(derecho(abb));

  if (abs(alturaIzq - alturaDer) < 2 && esAvl(izquierdo(abb)) && esAvl(derecho(abb)))
    return true;

  return false;
}

nat fibbonacci(int n)
{
  if (n < 2)
    return n;
  else
...
TAbb crearFibbonacci(int pos, int h)
{
  if (h < 1)
    return NULL;
  return consAbb(crearInfo(fibbonacci(h - 1) + 1 + pos, 0), crearFibbonacci(pos, h - 1), crearFibbonacci(fibbonacci(h - 1) + 1 + pos, h - 2));
}

TAbb avlMin(nat h)
{
  return crearFibbonacci(0, h);
}
...
TCadena filtradaOrdenada(TCadena cad, TIterador it)
{
  ...
  {
    nat max = cantidadEnCadena(cad);
    TMapping map = crearMap(max);
    for (nat i = 0; i < max; i++)
    {
      TInfo elem = primeroEnCadena(cad);
      if (esClaveEnMap(natInfo(elem), map))
      {
        double valor = realInfo(elem) + valorEnMap(natInfo(elem), map);
        map = desasociarEnMap(natInfo(elem), map);
        map = asociarEnMap(natInfo(elem), valor, map);
      }
      else
        map = asociarEnMap(natInfo(elem), realInfo(elem), map);
...
bool esPerfecto(TAbb abb)
{
  if (abb == NULL)
    ...
    return esPerfecto(izquierdo(abb)) && esPerfecto(derecho(abb)) && altura(izquierdo(abb)) == altura(derecho(abb));
}
